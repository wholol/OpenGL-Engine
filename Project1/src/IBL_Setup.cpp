...
IBL_Setup::IBL_Setup(const std::string& img_name, int diffuse_irr_texWidth, int diffuse_irr_texHeight,
	...
	,BRDF_intergral_texHeight(BRDF_intergral_texHeight) , specular_texHeight(specular_texHeight) , specular_texWdith(specular_texWdith)
	,img_name(img_name)
{
...
void IBL_Setup::diffuse_irradiance_init()
{
	...
	diffuse_irradiance_cubemap.genCubeMap(diffuse_irr_texWidth, diffuse_irr_texHeight);
...
void IBL_Setup::specular_mipmap_init()
{
	...
	for (int i = 0u; i < num_mipmaps; ++i)
	{
		float roughness = (float)i / (float)(num_mipmaps - 1);
		float mipmap_width = specular_texWdith * std::pow(0.5, i);
		float mipmap_height = specular_texHeight * std::pow(0.5, i);
		...
		for (int i = 0u; i < 6; ++i)
		{
			...
			GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, specular_prefilter_cubemap.getTexID(), i);
