	...
	:cubemap(cubemap)
{
	...
	VertexBuffer SkyBoxVBO(sizeof(skyboxVertices), skyboxVertices, 8 * sizeof(float));
	this->SkyBoxVBO = SkyBoxVBO;
	this->SkyBoxVBO.addattriblayout(3, GL_FLOAT, GL_FALSE);
	this->SkyBoxVBO.addattriblayout(3, GL_FLOAT, GL_FALSE);
	this->SkyBoxVBO.addattriblayout(2, GL_FLOAT, GL_FALSE);
...
void SkyBox::setCubeMapTexture(Texture& other)
{
	cubemap.ID = other.getTexID();
}
